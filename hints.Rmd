...
This example illustrates a multivariable logistic regression model using the `svyglm()` function from `survey`. The first argument is a modeling formula, the second is the survey object, and the third specifies a binary response.
...
To assess for differences across modality groups, I'll create a survey object `hints5_3_grp` that uses `nwgt0` as the sampling weight and `nwgt1` through `nwgt150` as replicate weights.
...
### Assessing for differences across groups, binary outcome

To fit a regression model we use `svyglm()` from `survey`. The first argument is a modeling formula; the second is the survey object. For a logistic regression model use the argument `family = "quasibinomial"`. Store the results in an object so that you can extract fitted values, residuals, and model metrics.
...
Note that "Paper only" is the referent because it's the first level of `Treatment_H5C3`. The results indicate no significant differences in `SeekCancerInfo` between the treatment groups. We can also test for the effect of `Treatment_H5C3` with the function `regTermTest()`, which tests the significance of a term or terms in a model.

```{r}
regTermTest(model01, "Treatment_H5C3")
```

### Assessing for differences across groups, continuous outcome
...
```{r}
# ADD EXAMPLE MODEL WITH PREDICTORS
# model00 <- svyglm(SeekCancerInfo ~ Treatment_H5C3 + [INSERT CONVARIATES],
#                   hints5_3_grp, 
#        family = "quasibinomial") 
# summary(model00)
```

The survey function `svychisq()` currently supports only two-way tables.

ADD EXAMPLE USING regTermtest() to assess group differences
...
hints5_3_paper <- hints5_3 %>% 
  ...
  as_survey_rep(weights = "TG1_FINWT0",
                repweights = paste0("TG1_FINWT", 1:50), # df = 49,
                type = "JK1", scale = 49/50, mse = TRUE)
nrow(hints5_3_paper)
# ADD EXAMPLE MODEL WITH PREDICTORS
...
## Group Diffferences Are Significant

In this case the group differences are not significant. If they were, one approach would be to control for group differences by including `Treatment_H5C3` in the model of interest. The appropriate survey object uses the `hints5_3_grp` object with 150 replicate weights. 

## Group Differences Not Significant

If group differences are not significant we can use the sample including all three groups. We need a different set of weights. For estimates using all groups use the weights `TG_all_FINWT0` through `TG_all_FINWT50`.
